'use strict';
// RK GATE — второй поток: та же логика, что rk_node.js, НО вход только в режиме
// расширения рыночной волатильности (гейт из бэктеста). Порт 8081, свой лог.
const http = require('http');
const fs = require('fs');
const BASE = 'https://fapi.binance.com';
const CFG = { rank:168, brk:24, holdMs:8*3600*1000, q:0.20, fee:0.0009,
  sessThr:0.45, weekendThr:0.30, topN:150, minVol:5e6,
  pollSec:30, rankMin:60, klimit:200, conc:8, port:8081,
  volSmooth:24, volLookback:168 };   // гейт: сглаживание 24ч, сравнение с 168ч назад
const STATE_FILE = '/root/rk_gate_state.json';
let S = { started:Date.now(), uni:[], data:{}, armed:{}, leaders:[], laggers:[],
  open:[], trades:[], bal:0, lastRank:0, lastErr:null, price:{}, updated:0, stocks:0,
  mvol:[], lastSnap:{}, lastSnapHour:0, gateOn:false, skippedGate:0 };
...
function save(){ if(saveTimer)return; saveTimer=setTimeout(()=>{ saveTimer=null;
  try{ if(S.trades.length>2000)S.trades=S.trades.slice(-2000); if(S.mvol.length>500)S.mvol=S.mvol.slice(-500); fs.writeFileSync(STATE_FILE, JSON.stringify(S)); }catch(e){console.log('save err',e.message);} },500); }
...
function classify(k){
  ...
  return (sess>=CFG.sessThr) || ((wdayAvg>0?wendAvg/wdayAvg:1) < CFG.weekendThr);
}
async function refreshRank(){
  ...
  console.log(`rank: crypto ${rows.length}, stocks ${stocks}, leaders ${S.leaders.length}, gate ${S.gateOn?'ON':'off'}`); save();
}
// --- рыночная волатильность: раз в час снимок цен -> среднее |часовой ход| -> сглаживание -> расширяется? ---
function updateMarketVol(price){
  const hour = Math.floor(Date.now()/3600000);
  if(hour===S.lastSnapHour) return;
  const prev=S.lastSnap;
  if(Object.keys(prev).length){
    let sum=0,cnt=0;
    for(const sym in price){ const p0=prev[sym]; if(p0>0 && price[sym]>0){ sum+=Math.abs(price[sym]/p0-1); cnt++; } }
    if(cnt>10){ S.mvol.push({t:Date.now(), v:sum/cnt}); }
  }
  S.lastSnap = {...price}; S.lastSnapHour = hour;
  // сглаживание и гейт
  const v=S.mvol;
  if(v.length>=CFG.volSmooth+CFG.volLookback){
    const sm=(a,i)=>{ let s=0; for(let k=i-CFG.volSmooth+1;k<=i;k++) s+=a[k].v; return s/CFG.volSmooth; };
    const nowSm=sm(v,v.length-1), pastSm=sm(v,v.length-1-CFG.volLookback);
    S.gateOn = nowSm > pastSm;
  }
  save();
}
async function poll(){
  try{
    ...
    for(const sym of S.leaders){ const d=S.data[sym], p=price[sym]; if(!d||p==null||inTr.has(sym))continue;
      if(S.armed[sym] && p>d.hh){ if(S.gateOn){ S.open.push({sym,dir:'LONG',entry:d.hh,ts:now,mom:d.mom}); inTr.add(sym); } else { S.skippedGate++; } S.armed[sym]=false; } }
    for(const sym of S.laggers){ const d=S.data[sym], p=price[sym]; if(!d||p==null||inTr.has(sym))continue;
      if(S.armed[sym] && p<d.ll){ if(S.gateOn){ S.open.push({sym,dir:'SHORT',entry:d.ll,ts:now,mom:d.mom}); inTr.add(sym); } else { S.skippedGate++; } S.armed[sym]=false; } }
    ...
    S.open = S.open.filter(t=>{ const p=price[t.sym]; if(p==null)return true;
      if(now-t.ts < holdMs) return true;
...
http.createServer((req,res)=>{
  ...
  const view={ stream:'GATE (vol-expansion)', uptimeMin:Math.round((Date.now()-S.started)/60000),
    updatedSecAgo:S.updated?Math.round((Date.now()-S.updated)/1000):null,
    gateOn:S.gateOn, volSamples:S.mvol.length, skippedByGate:S.skippedGate,
...
}).listen(CFG.port, ()=>console.log('GATE status http on :'+CFG.port));
async function main(){
